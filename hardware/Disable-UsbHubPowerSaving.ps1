...
ForEach ($Hub in $USBHubs) {
    ...
    $VarPowerSettings = $PowerMgmt | Where-Object {$_.InstanceName -like "*$($Hub.DeviceID)*"}
    If (($VarPowerSettings | Measure-Object).Count -eq 1) {
        If (($VarPowerSettings | Select-Object -ExpandProperty Enable) -eq $False) {Write-Host -Object "USB Hub '$($Hub.Name)' already has power saving disabled" -ForegroundColor Green}
        Else {
            Try {
    ...
    ElseIf (($VarPowerSettings | Measure-Object).Count -gt 1) {Write-Warning -Message "More than one WMI object representing power settings was found for '$($Hub.Name)', no settings have been changed for this device"}
